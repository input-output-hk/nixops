...
let
  ...
  # The following block is the terraform-provider package approach:
  #   - data.nix contains an attribute set of providers/plugins with values to be used with fetchfromGitHub
  #   - data.nix is generated by an update-all script which can be run occassionally and when run:
  #     - Parses github repos for available plugin release tags to update data.nix with
  #     - Uses a all-plugins.txt syntax file to choose which github repos to parse for tags
  #
  # Ref: https://github.com/NixOS/nixpkgs/tree/master/pkgs/applications/networking/cluster/terraform-providers

  allPlugins = import ./data.nix;
  toSrc = v:
...
#   python2Packages.buildPythonApplication rec {
    python2Packages.buildPythonPackage rec {
      ...
      # The myPy package causes a syntax error pluggy hookspec
      # We should be able to exclude the offending pluggy lines from myPy and
      # add myPy back in.  The file setup.cfg will not be needed until then.
...
#     nativeBuildInputs = [ pkgs.mypy ];
      ...
      propagatedBuildInputs = with python2Packages;
        [ prettytable
          ...
          (import ((toString (toSrc v)) + "/release.nix") {}).build.${system})
          (p allPlugins);


      # The map function and its arguments above accomplish a few tasks:
      #   - Uses a function "p", passed as an argument to release.nix to filter all plugins (allPlugins from data.nix)
      #   - The function "p" is of the pattern (p: [ p.plugin1 p.plugin2 ... ])
      #   - The source for the filtered plugins is fetched and then build from the release.nix file as a runtime dependency
      #
      # Overrides (override[Attrs|Derivation]) are not yet implemented as they are in terraform
      # Ref: https://github.com/NixOS/nixpkgs/blob/master/pkgs/applications/networking/cluster/terraform/default.nix
      ...
      # Reenable myPy once hookspec error is excluded
